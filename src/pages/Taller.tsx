...
export const Talleres: React.FC = () => {
    ...
    const footerDialog = (
        <div>
            ...
            <Button label='Guardar' icon='pi pi-save' className='p-button-outlined p-button-success' onClick={saveTaller}></Button>
        </div>
    );
    ...
    const handleDelete = async (id: number) => {
        try {
            ...
            toast.current?.show({ 
                severity: 'success', 
                summary: 'Éxito', 
                detail: 'Taller eliminado correctamente', life: 3000 
            });
            loadTalleres();
        } catch (error) {
    ...
    return (
        <div>
            ...
            <Button label='Nuevo Taller' icon='pi pi-plus' className='p-button-outlined p-button-primary' onClick={openNew}></Button>
            ...
            <DataTable value={talleres}>
                ...
                <Column field="especialidades" header="Especialidades" sortable></Column>
                <Column header="Acciones" body={(rowData: Taller) => (
                    <>
                        <Button icon='pi pi-pencil' className='p-button-rounded p-button-outlined p-button-success p-mr-2' onClick={() => { setEditingTaller(rowData); setDisplayDialog(true); }}></Button>
            ...
            <Dialog visible={displayDialog} header={editingTaller.id ? 'Editar Taller' : 'Nuevo Taller'} onHide={hideDialog} footer={footerDialog}>
                <div className='p-field'>
                    <label htmlFor='nombre'>Nombre: </label>
                    <InputText id='nombre' value={editingTaller.nombre} onChange={(e) => setEditingTaller({ ...editingTaller, nombre: e.target.value })} required autoFocus className={submitted && !editingTaller.nombre ? 'p-invalid' : ''} />
                    {submitted && !editingTaller.nombre && <small className="p-error">Nombre es requerido.</small>}
                </div><br />
                <div className='p-field'>
                    <label htmlFor='direccion'>Dirección: </label>
                    <InputText id='direccion' value={editingTaller.direccion} onChange={(e) => setEditingTaller({ ...editingTaller, direccion: e.target.value })} required className={submitted && !editingTaller.direccion ? 'p-invalid' : ''} />
                    {submitted && !editingTaller.direccion && <small className="p-error">Dirección es requerida.</small>}
                </div><br />
                <div className='p-field'>
                    <label htmlFor='telefono'>Teléfono: </label>
                    <InputText id='telefono' value={editingTaller.telefono} onChange={(e) => setEditingTaller({ ...editingTaller, telefono: e.target.value })} required className={submitted && !editingTaller.telefono ? 'p-invalid' : ''} />
                    {submitted && !editingTaller.telefono && <small className="p-error">Teléfono es requerido.</small>}
                </div><br />
                <div className='p-field'>
                    <label htmlFor='correo'>Correo: </label>
                    <InputText id='correo' value={editingTaller.correo} onChange={(e) => setEditingTaller({ ...editingTaller, correo: e.target.value })} required className={submitted && !editingTaller.correo ? 'p-invalid' : ''} />
                ...
                <div className='p-field'>
                    ...
                    <InputText id='horariosAtencion' value={editingTaller.horariosAtencion} onChange={(e) => setEditingTaller({ ...editingTaller, horariosAtencion: e.target.value })} />
                </div><br />
                <div className='p-field'>
                    <label htmlFor='especialidades'>Especialidades: </label>
                    <InputText id='especialidades' value={editingTaller.especialidades} onChange={(e) => setEditingTaller({ ...editingTaller, especialidades: e.target.value })} />
                </div><br />
            </Dialog>
        </div>
    );
